...
unpack_kitti_file() {
  ...
  wget https://s3.eu-central-1.amazonaws.com/avg-kitti/raw_data/"$1"/"$2" && unzip "$2"  -d "$3"
}
...
unpack_kitti_calib_file() {
  ...
  wget https://s3.eu-central-1.amazonaws.com/avg-kitti/raw_data/"$calib_file" && unzip "$calib_file"  -d "$2"
}
